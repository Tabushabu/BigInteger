...
{ // only I decide when you are done
...
{
    printf("Beginning BigIntegerTests. . .\n");
    ...
    for (uint8_t i = FIRST_TEST; i < NUM_TESTS; i++) {
        ...
        printf("test = %d --- testStatus = %d\n", i - FIRST_TEST, testStatus = runTest(&A, &B, &C, &D, i));
    }
    freeBigInteger(&A);
    freeBigInteger(&B);
    freeBigInteger(&C);
